 ...
 */
  ...
  * Outputs:		joint_angle[9]		9 joint angles(degree)
  */
 void SwiftproState_Callback(const swiftpro::msg::SwiftproState& msg)
 {
	 float position[3];
	 float angle[3];
	 ...
	 if ( swiftpro_ik(position, angle) )
		 ...
		 RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Inverse kinematic is wrong");
 ...
 {	
	 ...
	 auto sub = node->create_subscription<swiftpro::msg::SwiftproState>("SwiftproState_topic", 1, SwiftproState_Callback);
	 ...
	 while (rclcpp::ok())
	 {
		 ...
		 odom_trans.header.stamp = node->get_clock()->now();
